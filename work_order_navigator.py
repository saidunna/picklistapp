...
st.markdown(
    ...
    .title {
        font-size: 38px;
        color: #EA481F; /* Fresh green for the title */
        font-weight: bold;
    }
...
if 'site_id' in st.session_state:
    ...
    if work_order_id:
        ...
        if not filtered_pick_list.empty:
            ...
            st.subheader(f"Work Order Details: {work_order_id}")
            ...
            if not filtered_pick_list.empty:
                ...
                def generate_pdf():
                    pdf = FPDF()
                    # Add a Unicode font (DejaVu) to avoid text errors
                    ...
                    for item_id, item_rows in grouped_data.items():
                        ...
                        pdf.cell(40, 10, f"Item ID: ", border=0, ln=False)
                        pdf.set_font("DejaVu", '', 10)
                        pdf.cell(0, 10, f"{item_id}", ln=True)
